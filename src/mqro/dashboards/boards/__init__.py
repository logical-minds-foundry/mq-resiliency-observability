...
- :mod:`~mqro.dashboards.boards.primitives` — the Grafana panel toolkit (stat/timeseries/table/
  logs/state-timeline builders, the colour mappings, the row-key normalizer). Ported verbatim from
  the lab's ``clusterboard`` primitives; the only change is that datasource UIDs are always passed
  in, never assumed.
